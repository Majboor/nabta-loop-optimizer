...
MULTIOBJECTIVE VARIANT (experiment/multiobjective branch): the element
*mix* is now part of the genome — each of the 24 budget slots evolves
both its position on the loop and its type (pergola or tree cluster) —
and the fitness gains a water-demand penalty: tree clusters cost
irrigation, pergolas do not. The GA now negotiates the shade-vs-water
trade-off the 43 entry variants were scored on.
...
Outputs: assets/multiobjective.png, assets/multiobjective_layout.png
"""
...
# ---------------------------------------------------------------------------
# Shade-element budget (fixed count; the pergola/tree mix now evolves)
# ---------------------------------------------------------------------------
N_ELEMENTS = 24
N_GENES = 2 * N_ELEMENTS    # positions [0:24] + type genes [24:48] (>0.5 = tree)
...
W_WATER = 0.08          # weight of the water-demand penalty (all-tree layout
                        # pays the full 0.08; each cluster costs 0.08/24 of
                        # fitness ~ 3.3 m of shaded path it must earn back)
...
# ---------------------------------------------------------------------------
def split_genome(pop: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """(positions (n,24) in [0,1), is_tree (n,24) bool) from a (n,48) genome."""
    pop = np.atleast_2d(pop)
    return pop[:, :N_ELEMENTS] % 1.0, pop[:, N_ELEMENTS:] > 0.5


def shaded_mask(pop: np.ndarray) -> np.ndarray:
    """Boolean (pop, M): which path samples are shaded, per individual.

    Genome: (pop, 48) floats — 24 arc-length fractions along the loop plus
    24 type genes (>0.5 = tree cluster, else pergola). A path sample is
    shaded when it lies inside any element footprint displaced by the sun
    vector; each slot uses the footprint of its evolved type.
    """
    pos, is_tree = split_genome(pop)
    idx = (pos * M).astype(int) % M                    # (n, 24)

    # --- pergola hypothesis: rectangles aligned with the path tangent ------
    c = P[idx] + OFF_PERGOLA                           # shadow centres (n,24,2)
    t = T[idx]                                         # orientations   (n,24,2)
    n = NRM[idx]
    d = P[None, None, :, :] - c[:, :, None, :]         # (n,24,M,2)
    u = np.einsum("nkmc,nkc->nkm", d, t)               # along-axis coord
    v = np.einsum("nkmc,nkc->nkm", d, n)               # across-axis coord
    perg = (np.abs(u) <= PERGOLA_LEN / 2) & (np.abs(v) <= PERGOLA_WID / 2)

    # --- tree hypothesis: displaced canopy disks ----------------------------
    c = P[idx] + OFF_TREE                              # (n,24,2)
    d = P[None, None, :, :] - c[:, :, None, :]
    tree = (d ** 2).sum(-1) <= TREE_RADIUS ** 2        # (n,24,M)

    # each slot contributes the mask of its evolved type
    return np.where(is_tree[:, :, None], tree, perg).any(axis=1)
...
def clustering_penalty(pop: np.ndarray) -> np.ndarray:
    ...
    pos, _ = split_genome(pop)
    s = np.sort(pos, axis=1) * LOOP["length"]
...
def water_demand(pop: np.ndarray) -> np.ndarray:
    """(pop,) fraction of the budget spent on irrigated canopy (trees)."""
    _, is_tree = split_genome(pop)
    return is_tree.mean(axis=1)
...
def evaluate(pop: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    fit = (shade - W_CLUSTER * clustering_penalty(pop)
           - W_WATER * water_demand(pop))
    return fit, shade
...
def run_ga(rng: np.random.Generator,
    ...
    hist_best, hist_mean, hist_best_shade, hist_best_trees = [], [], [], []
    ...
    for _ in range(generations):
        ...
        hist_best_trees.append(int(split_genome(pop[b:b + 1])[1].sum()))
...
def draw_layout(ax, genome: np.ndarray, title: str, shade_frac: float,
    ...
    pos, is_tree = split_genome(genome[None, :])
    idx, is_tree = (pos[0] * M).astype(int) % M, is_tree[0]
...
def plot_multiobjective(res: dict, path: str) -> None:
    """Two panels: fitness convergence, and how the best individual spends
    its budget (shade won vs irrigated tree clusters kept)."""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12.5, 4.6))
    g = np.arange(1, len(res["hist_best"]) + 1)

    ax1.plot(g, res["hist_best"], color=BLUE, lw=2, label="best fitness")
    ax1.plot(g, res["hist_mean"], color=AQUA, lw=2, label="mean fitness")
    ax1.annotate(f"{res['hist_best'][-1]:.3f}", (g[-1], res["hist_best"][-1]),
                 xytext=(4, 2), textcoords="offset points", fontsize=8.5,
                 color=BLUE)
    ax1.set_xlabel("generation")
    ax1.set_ylabel("fitness  (shade − clustering − water)")
    ax1.set_title("Convergence with water-demand penalty", color=INK)
    ax1.grid(True, axis="y")
    ax1.spines[["top", "right"]].set_visible(False)
    ax1.legend(frameon=False, loc="lower right")

    ax2.plot(g, res["hist_best_shade"] * 100, color=BLUE, lw=2,
             label="shaded fraction (%)")
    ax2.plot(g, res["hist_best_trees"] / N_ELEMENTS * 100, color=AQUA, lw=2,
             label="budget on irrigated trees (%)")
    ax2.set_xlabel("generation")
    ax2.set_ylabel("percent")
    ax2.set_title("Best individual: shade vs irrigation demand", color=INK)
...
def main() -> None:
    ...
    print(f"GA optimized    : {res['best_shade'] * 100:.1f}% shaded, "
          f"{res['best_trees']}/{N_ELEMENTS} irrigated tree clusters "
          f"(fitness {res['best_fitness']:.3f}) in {dt:.1f}s")

    import os
    os.makedirs("assets", exist_ok=True)

    plot_multiobjective(res, "assets/multiobjective.png")

    fig, ax = plt.subplots(figsize=(9.5, 7))
    draw_layout(ax, res["best"],
                f"Multiobjective layout ({res['best_trees']} trees / "
                f"{N_ELEMENTS - res['best_trees']} pergolas)",
                res["best_shade"], rooms=garden_rooms(LOOP))
    handles = [
        plt.Line2D([], [], color=C_PERGOLA, lw=1.6, label="pergola (12 m)"),
        plt.Line2D([], [], color=C_TREE, lw=1.6, label="tree cluster (irrigated)"),
    ...
    fig.tight_layout()
    fig.savefig("assets/multiobjective_layout.png", dpi=160)
    plt.close(fig)

    print("wrote assets/multiobjective.png, assets/multiobjective_layout.png")
